...
def _latest_stamp() -> str:
    ...
    if not stamps:
        raise FileNotFoundError(f"No full runs found under {RUN_ROOT}")
    return stamps[-1]


def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize Murata affine full results")
    parser.add_argument("--stamp", default=None, help="Full run stamp under runs/murata_affine_full")
    ...
    print(f"=== Murata Affine Full Report | stamp={stamp} ===")
