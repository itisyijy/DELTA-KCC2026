...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize Murata selective-activation sweep results")
    parser.add_argument("--stamp", default=None, help="Sweep stamp under runs/murata_selective_activation_sweep")
    ...
    print(f"=== Murata Selective-Activation Sweep Report | stamp={stamp} ===")
    for name, avg, diag in sorted(rows, key=lambda row: row[1]["mse"]):
        print(
            f"{name:16s} "
            f"MSE={avg['mse']:.4f} MAE={avg['mae']:.4f} sMAPE={avg['smape']:.2f}% "
            f"adapt={diag['adapt_rate']:.3f} accept={diag['accept_gate_skip_rate']:.3f} "
            f"rollback={diag['rollback_skip_rate']:.3f}"
        )
