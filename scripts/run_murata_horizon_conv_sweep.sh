...
RUN_ROOT="$ROOT/runs/murata_horizon_conv_sweep"
LOG_DIR="$ROOT/logs"
...
write_cfg() {
  ...
  "$PYTHON" - "$ROOT/configs/murata/fed_tta.yaml" "$ROOT/configs/murata/fed.yaml" "$dst" \
    "$prereq_dst" "$DEVICE" "$outdir" "$mode" "$hard_gate" <<'PY'
import math
import sys
import yaml

base_cfg, prereq_cfg, dst, prereq_dst, device, outdir, mode, hard_gate = sys.argv[1:]
...
cfg["tta"]["hard_gate_scale"] = float(hard_gate)
cfg["tta"]["hard_gate_min_history"] = 20 if float(hard_gate) > 0.0 else 0
...
launch() {
  ...
  session="mur_hconv_${STAMP}_${name}"
  mkdir -p "$outdir"
  write_cfg "$cfg" "$prereq_dst" "$outdir" "$mode" "$hard_gate"
...
cd "$ROOT"
echo "=== Murata Horizon-Conv Sweep | stamp=$STAMP | device=$DEVICE | max_workers=$MAX_WORKERS ==="
launch time_control time_affine 0.0
launch conv_base horizon_conv 0.0
launch conv_hgate1p10 horizon_conv 1.10
echo ""
echo "Launched 3 full-scale runs."
echo "Monitor:"
